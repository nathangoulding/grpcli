...
var repl = function () {
	...
	var command = function () {
		...
		var _printUsage = function () {
			var serviceMethods = _opts.service.service;
			Object.keys(serviceMethods).forEach(function (methodKey) {
				var child = serviceMethods[methodKey];
				console.log('%s(%s) {\n  return %s;\n}', (child.originalName.charAt(0).toLowerCase() + child.originalName.slice(1)).blue.bold, child.requestType.name, child.responseType.name);
			});
		};
